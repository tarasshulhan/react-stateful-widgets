...
export default function Programmers() {
  ...
  const [featuredProgrammer, setFeaturedProgramer] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    // programers.forEach(element => {
    //   if(element.id === featuredProgrammer){
    //     console.log(element);
    //     return element.name;
    //   }
    // })
    ...
    return programers.filter(elem => elem.id === featuredProgrammer)[0].name;
  };
      ...
      <div className='programmers'>
        {
          ...
          programers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setFeaturedProgramer(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
